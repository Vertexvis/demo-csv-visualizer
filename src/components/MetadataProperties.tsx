...
export function MetadataProperties({ metadata }: Props): JSX.Element {
  if (metadata == null) return <SelectItemMessage />;

  const propKeys = Object.keys(metadata.properties);
  if (propKeys.length === 0) return <SelectItemMessage />;
...
function SelectItemMessage(): JSX.Element {
  return <NoData>Select an item to view its metadata.</NoData>;
}
